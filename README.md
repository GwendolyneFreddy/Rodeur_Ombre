...
*Si une ancienne version de ce mod est déjà installée, il est nécessaire de la désinstaller d'abord. Pour cela, lancez **`setup-fr_ombre.exe`**, et désinstallez tous les composants précédemment installés. Une fois la désinstallation achevée, supprimez le :file_folder: répertoire **fr_ombre** et le fichier **setup-fr_ombre.exe** (version Windows) avant d'extraire la nouvelle version du mod.*
...
Le Rôdeur de l'Ombre pour Windows est livré et installé avec WeiDU, et est diffusé sous forme d'archive.

Vous devez extraire les fichiers de l'archive dans votre répertoire de jeu (*le dossier qui contient le fichier CHITIN.KEY*) à l'aide de <a href="http://www.7-zip.org/download.html">7zip</a> ou de <a href="http://www.rarlab.com/download.htm">WinRAR</a>. Une fois l'archive extraite correctement, vous devriez trouver le répertoire :file_folder: fr_ombre et le fichier setup-fr_ombre.exe dans votre répertoire de jeu. Pour installer le mod, il suffit de double-cliquer sur **`setup-fr_ombre.exe`** et de suivre les instructions affichées à l'écran.

Vous pouvez lancer **`setup-fr_ombre.exe`** dans votre répertoire de jeu pour réinstaller, désinstaller, ou encore changer des composants.
...
Tapez la commande **`export PATH=$PATH:/usr/local/bin`**, puis lancez **`weinstall setup-fr_ombre`** depuis le dossier de votre jeu pour installer le mod. Puis, lancez **`wine bgmain.exe`** (ou **`wine baldur.exe`** pour les versions EE) et commencez à jouer.

## 

#### Note pour effectuer une désinstallation complète

En plus des méthodes détaillées plus haut pour supprimer des composants, il est possible de désinstaller complètement le mod en tapant **`setup-fr_ombre --uninstall`** dans une ligne de commandes, ce qui supprimera tous les composants sans devoir ingurgiter tous les messages.</br>
<div align="right"><a href="#top">Retour en haut de page</a></div>
...
- Completed kit description with missing features and requirements. Added lower cased kit name.
